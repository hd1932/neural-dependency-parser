...
class ParserModel(nn.Module):
    ...
    PyTorch Notes:
        - Note: "ParserModel" is a subclass of the "nn.Module" class. In PyTorch all neural networks
        ...
        - Other methods of ParserModel can access variables that have "self." prefix. Thus,
            we add the "self." prefix layers, values, etc. that we want to utilize
            in other ParserModel methods.
        - For further documentation on "nn.Module" see https://pytorch.org/docs/stable/nn.html.
    """
    def __init__(self, embeddings, n_features=36,
        ...
        ###     1) Declares `self.embed_to_hidden_weight` and `self.embed_to_hidden_bias` as `nn.Parameter`.
        ###        Initializes weight with the `nn.init.xavier_uniform_` function and bias with `nn.init.uniform_`
        ###        with default parameters.
        ###     2) Constructs `self.dropout` layer.
        ###     3) Declares `self.hidden_to_logits_weight` and `self.hidden_to_logits_bias` as `nn.Parameter`.
        ###        Initializes weight with the `nn.init.xavier_uniform_` function and bias with `nn.init.uniform_`
        ###        with default parameters.
        ###
    ...
    def embedding_lookup(self, w):
        ...
        ###     1) For each index `i` in `w`, selects `i`th vector from self.embeddings
        ###     2) Reshapes the tensor using `view` function if necessary
        ###
    ...
    def forward(self, w):
        """ Run the model forward.
            ...
            Note: will not apply the softmax function here because it is included in the loss function nn.CrossEntropyLoss
            ...
            PyTorch Notes:
                ...
                - Useful doc: https://pytorch.org/docs/stable/nn.html#torch.nn.Module.forward
        ...
        """
        ###     Completes the forward computation as described in write-up. In addition, include a dropout layer

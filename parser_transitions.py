...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        """
        ###     Implements a single parsing step, i.e. the logic for the following as:
        ###         1. Shift
        ###         2. Left Arc
        ###         3. Right Arc
        ...
        elif transition in ("LA", "RA") and len(self.stack) > 1:
            head, dependent = (self.stack[-2], self.stack[-1]) if transition == "RA" else (self.stack[-1], self.stack[-2])
            self.dependencies.append((head, dependent))
            self.stack.pop(-2 if transition == "LA" else -1)
